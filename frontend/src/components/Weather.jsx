...
const Weather = ({ apiData, setApiData }) => {
...
useEffect(() => {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      // Fetch weather data from the updated API - FIXED: Use template literals
      fetch(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=wind_speed_10m&hourly=temperature_2m,rain,weather_code,soil_temperature_0cm,soil_temperature_6cm,soil_temperature_18cm,soil_temperature_54cm`)
        ...
        .then(data => {
          ...
          // Fetch location data after weather data is available
          fetch(`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${latitude}&longitude=${longitude}&localityLanguage=en`)
            .then(response => response.json())
            .then(locationData => {
              const loc = `${locationData.city}, ${locationData.principalSubdivision}, ${locationData.countryName}`;
              setLocation(loc);

              // Set apiData globally after all data is available
              const newApiData = {
                location: loc,
              ...
              console.log('Setting apiData:', apiData);
            })
            .catch(error => {
